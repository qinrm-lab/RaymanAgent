...
BATS_BIN="$(resolve_bats_bin || true)"
...
set +e
if "${BATS_BIN}" --help 2>&1 | grep -q -- '--report-formatter'; then
  "${BATS_BIN}" --report-formatter junit --output "${RUNTIME_DIR}" "${TEST_DIR}" | tee "${LOG_PATH}"
else
  "${BATS_BIN}" "${TEST_DIR}" | tee "${LOG_PATH}"
fi
rc=$?
set -e
