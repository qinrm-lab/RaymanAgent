...
- `setup.ps1` 保留全量资产生成，同时会在末尾自动注入 SCM 忽略规则（默认写入 `.git/info/exclude`，仅本地生效）。
- 默认会忽略 `.Rayman/.dist/`、`.Rayman/scripts/`、`.Rayman/runtime/`、`.vscode/tasks.json`、`.cursorrules`、`.clinerules` 等高噪声生成产物。
- setup 末尾会输出 `modified/added/untracked/total` 统计；超过软阈值（默认 `100`）时，会提示主要来源目录。
- 若文件已被 Git 跟踪，ignore 规则不会生效，需要先手动取消跟踪（例如 `git rm --cached <path>`）。
...
- 系统能力精简（保守映射）默认仅覆盖 `dispatch`：当 Codex 版本满足门槛时，自动委派到 `codex exec -C <WorkspaceRoot> <prompt>`
- `review-loop` 仍保持 Rayman 原有“dispatch + test-fix”闭环，不替换为 `codex review`
- `setup/init` 会产出 `.Rayman/runtime/system_slim.report.md`，并在检测到 VSCode/Codex 升级时提示是否继续扩大精简范围
- `rayman review-loop --TaskKind review`：执行 “dispatch + test-fix” 闭环，并沉淀 first-pass 指标
...
- `RAYMAN_SCM_IGNORE_MODE`（默认 `info-exclude`）：SCM 忽略规则注入目标（`info-exclude|gitignore|off`）。
