...
4. `install-local` 会优先读取 `RAYMAN_WORKER_VSDBG_PATH`；若未预置，则自动把 `vsdbg` 安装到 `.Rayman/tools/vsdbg/`。
...
5. 先确认 `worker status` 中 `debugger_ready=true`
6. 执行远端命令：`.\.Rayman\rayman.ps1 worker exec dotnet build`
...
1. 先执行 `.\.Rayman\rayman.ps1 worker status`，确认 `debugger_ready=true`，失败时查看 `debugger_error`
