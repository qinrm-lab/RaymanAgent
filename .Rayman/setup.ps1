...
$legacyCleanupScript = Join-Path $PSScriptRoot 'scripts\utils\legacy_rayman_cleanup.ps1'
$playwrightReadyLibPath = Join-Path $PSScriptRoot 'scripts\pwa\playwright_ready.lib.ps1'
$runtimeCleanupImported = $false
$legacyCleanupImported = $false
$playwrightReadyLibImported = $false
...
if (Test-Path -LiteralPath $runtimeCleanupScript -PathType Leaf) {
    ...
    $runtimeCleanupImported = $true
}
if (Test-Path -LiteralPath $legacyCleanupScript -PathType Leaf) {
    . $legacyCleanupScript -NoMain
    $legacyCleanupImported = $true
}
if (Test-Path -LiteralPath $playwrightReadyLibPath -PathType Leaf) {
    . $playwrightReadyLibPath
    $playwrightReadyLibImported = $true
}
...
function Invoke-PlaywrightAutoInstall {
    ...
    if ($shouldHost) {
        $attempted = $true
        if (-not $playwrightReadyLibImported -or -not (Get-Command Resolve-RaymanPlaywrightHostInstallInvocation -ErrorAction SilentlyContinue)) {
            Write-Host "⚠️ [Playwright][host] 缺少 Playwright host invoker helper，跳过 host 自动安装。" -ForegroundColor Yellow
        } else {
            $hostInvocation = Resolve-RaymanPlaywrightHostInstallInvocation -WorkspaceRoot $WorkspaceRoot -Browser $browserNorm
            if (-not [bool]$hostInvocation.success) {
                $hostFailureDetail = if (-not [string]::IsNullOrWhiteSpace([string]$hostInvocation.detail)) { [string]$hostInvocation.detail } else { 'host playwright invoker resolve failed' }
                if ($hostInvocation.PSObject.Properties['install_output'] -and @($hostInvocation.install_output).Count -gt 0) {
                    $tail = (@($hostInvocation.install_output | ForEach-Object { [string]$_ } | Select-Object -Last 3) -join ' | ').Trim()
                    if (-not [string]::IsNullOrWhiteSpace([string]$tail)) {
                        $hostFailureDetail = ("{0}; tail={1}" -f $hostFailureDetail, $tail)
                    }
                }
                Write-Host ("⚠️ [Playwright][host] 自动安装准备失败：{0}" -f $hostFailureDetail) -ForegroundColor Yellow
            } else {
                $hostLabel = if ([string]::IsNullOrWhiteSpace([string]$hostInvocation.install_source)) { 'host' } else { [string]$hostInvocation.install_source }
                Write-Host ("   -> [host:{0}] {1}" -f $hostLabel, [string]$hostInvocation.command_display) -ForegroundColor DarkCyan
                try {
                    $hostInstallResult = Invoke-SetupNativeCommandCapture -FilePath ([string]$hostInvocation.command) -ArgumentList @($hostInvocation.argument_list)
                    $hostInstallExitCode = [int]$hostInstallResult.ExitCode
                    $hostHasFatalOutput = Test-PlaywrightAutoInstallOutputHasFatalError -OutputLines @($hostInstallResult.OutputLines)
                    if ($hostInstallExitCode -eq 0 -and -not $hostHasFatalOutput) {
                        Write-Host ("✅ [Playwright][host] 自动安装完成（source={0}）" -f $hostLabel) -ForegroundColor Green
                    } elseif ($hostHasFatalOutput) {
                        Write-Host ("⚠️ [Playwright][host] 自动安装输出命中 fatal 标记（exit={0}），将继续执行就绪检查。" -f $hostInstallExitCode) -ForegroundColor Yellow
                    } else {
    ...
    if ($hostIsWindows) {
        if ($shouldWsl -and (Get-Command wsl.exe -ErrorAction SilentlyContinue)) {
            ...
            if (Test-Path -LiteralPath $wslEnsureScript -PathType Leaf) {
                ...
                try {
                    $wslInstallResult = Invoke-SetupNativeCommandCapture -FilePath 'wsl.exe' -ArgumentList @('-e', 'bash', '-lc', $cmd)
                    $wslInstallExitCode = [int]$wslInstallResult.ExitCode
                    $wslHasFatalOutput = Test-PlaywrightAutoInstallOutputHasFatalError -OutputLines @($wslInstallResult.OutputLines)
                    if ($wslInstallExitCode -eq 0 -and -not $wslHasFatalOutput) {
                        Write-Host "✅ [Playwright][wsl] 自动安装完成" -ForegroundColor Green
                    } elseif ($wslHasFatalOutput) {
                        Write-Host ("⚠️ [Playwright][wsl] 自动安装输出命中 fatal 标记（exit={0}），将继续执行就绪检查。" -f $wslInstallExitCode) -ForegroundColor Yellow
                    } else {
        ...
        $shouldLinux = ($scopeNorm -eq 'all' -or $scopeNorm -eq 'wsl')
        if ($scopeNorm -eq 'host') {
            $attempted = $true
            if (-not $playwrightReadyLibImported -or -not (Get-Command Resolve-RaymanPlaywrightHostInstallInvocation -ErrorAction SilentlyContinue)) {
                Write-Host "⚠️ [Playwright][host] 缺少 Playwright host invoker helper，跳过 host 自动安装。" -ForegroundColor Yellow
            } else {
                $hostInvocation = Resolve-RaymanPlaywrightHostInstallInvocation -WorkspaceRoot $WorkspaceRoot -Browser $browserNorm
                if (-not [bool]$hostInvocation.success) {
                    Write-Host ("⚠️ [Playwright][host] 自动安装准备失败：{0}" -f [string]$hostInvocation.detail) -ForegroundColor Yellow
                } else {
                    Write-Host ("   -> [host:{0}] {1}" -f [string]$hostInvocation.install_source, [string]$hostInvocation.command_display) -ForegroundColor DarkCyan
                    try {
                        $hostInstallResult = Invoke-SetupNativeCommandCapture -FilePath ([string]$hostInvocation.command) -ArgumentList @($hostInvocation.argument_list)
                        $hostInstallExitCode = [int]$hostInstallResult.ExitCode
                        $hostHasFatalOutput = Test-PlaywrightAutoInstallOutputHasFatalError -OutputLines @($hostInstallResult.OutputLines)
                        if ($hostInstallExitCode -eq 0 -and -not $hostHasFatalOutput) {
                            Write-Host ("✅ [Playwright][host] 自动安装完成（source={0}）" -f [string]$hostInvocation.install_source) -ForegroundColor Green
                        } elseif ($hostHasFatalOutput) {
                            Write-Host ("⚠️ [Playwright][host] 自动安装输出命中 fatal 标记（exit={0}），将继续执行就绪检查。" -f $hostInstallExitCode) -ForegroundColor Yellow
                        } else {
                            Write-Host ("⚠️ [Playwright][host] 自动安装失败（exit={0}），将继续执行就绪检查。" -f $hostInstallExitCode) -ForegroundColor Yellow
            ...
            if ((Get-Command bash -ErrorAction SilentlyContinue) -and (Test-Path -LiteralPath $wslScript -PathType Leaf)) {
                ...
                try {
                    $linuxInstallResult = Invoke-SetupNativeCommandCapture -FilePath 'bash' -ArgumentList @('./.Rayman/scripts/pwa/ensure_playwright_wsl.sh', '--browser', $browserNorm, '--require', '0')
                    $linuxInstallExitCode = [int]$linuxInstallResult.ExitCode
                    $linuxHasFatalOutput = Test-PlaywrightAutoInstallOutputHasFatalError -OutputLines @($linuxInstallResult.OutputLines)
                    if ($linuxInstallExitCode -eq 0 -and -not $linuxHasFatalOutput) {
                        Write-Host "✅ [Playwright][linux] 自动安装完成" -ForegroundColor Green
                    } elseif ($linuxHasFatalOutput) {
                        Write-Host ("⚠️ [Playwright][linux] 自动安装输出命中 fatal 标记（exit={0}），将继续执行就绪检查。" -f $linuxInstallExitCode) -ForegroundColor Yellow
                    } else {
...
function Get-SetupManagedWriteFailureMessage {
    param(
        [object]$Result,
        [string]$Path
    )

    $label = if (-not [string]::IsNullOrWhiteSpace([string]$Path)) { [string]$Path } else { [string]$Result.path }
    ...
    return ("{0} 写入失败（reason={1}）：{2}" -f $label, $reason, $detail)
}

function Invoke-SetupManagedFileWrite {
    param(
        [string]$Path,
    ...
    $displayPath = if ([string]::IsNullOrWhiteSpace($FailurePath)) { $Path } else { $FailurePath }
    $result = $null
    if (Get-Command Set-RaymanManagedUtf8File -ErrorAction SilentlyContinue) {
        $result = Set-RaymanManagedUtf8File -Path $Path -Content $Content -AuditRoot $WorkspaceRoot -Label $Label
    } else {
        try {
    ...
    if (-not [bool]$result.ok) {
        $message = Get-SetupManagedWriteFailureMessage -Result $result -Path $displayPath
        if ($Required) {
            throw ("managed-write-failed: {0}" -f $message)
        }
    ...
    switch ([string]$result.mode) {
        'unchanged' {
            if (-not [string]::IsNullOrWhiteSpace($UnchangedMessage)) {
                Write-Host $UnchangedMessage -ForegroundColor DarkCyan
            }
            break
        }
        'in_place_fallback' {
            ...
            } elseif (-not [string]::IsNullOrWhiteSpace($SuccessMessage)) {
                Write-Host $SuccessMessage -ForegroundColor Yellow
            }
            break
        }
        default {
...
function Invoke-SetupNativeCommandCapture {
    param(
        [string]$FilePath,
        [string[]]$ArgumentList = @()
    )
...
function Test-PlaywrightAutoInstallOutputHasFatalError {
    param(
        [string[]]$OutputLines = @()
    )
    ...
    return ($joined -match '(?im)^\s*npm\s+error\b')
}
...
function Ensure-WorkspaceEnvDefaults {
    ...
    $defaults = [ordered]@{
        ...
        'RAYMAN_INTERACTION_MODE' = 'detailed'
        'RAYMAN_POST_COMMAND_HYGIENE_ENABLED' = '1'
        'RAYMAN_ALERT_SURFACE' = 'log'
        'RAYMAN_ALERT_DONE_ENABLED' = '1'
        'RAYMAN_ALERT_SOUND_ENABLED' = '1'
        'RAYMAN_ALERT_SOUND_DONE_ENABLED' = '1'
        'RAYMAN_ALERT_TTS_ENABLED' = '0'
        'RAYMAN_ALERT_TTS_DONE_ENABLED' = '0'
        'RAYMAN_AUTO_START_ATTENTION_WATCH_ENABLED' = '1'
        'RAYMAN_ALERT_WATCH_ON_PROMPT_ENABLED' = '1'
        'RAYMAN_ALERT_WATCH_VSCODE_WINDOWS_ENABLED' = '1'
        'RAYMAN_AUTO_SAVE_WATCH_ENABLED' = '0'
        'RAYMAN_NETWORK_RESUME_WATCH_ENABLED' = '1'
        'RAYMAN_NETWORK_RESUME_THRESHOLD_SECONDS' = '1800'
        'RAYMAN_NETWORK_RESUME_THROTTLE_WAIT_SECONDS' = '300'
        'RAYMAN_NETWORK_RESUME_POLL_MS' = '5000'
        'RAYMAN_NETWORK_RESUME_PROBE_TIMEOUT_MS' = '5000'
        'RAYMAN_NETWORK_RESUME_RETRY_SECONDS' = '300'
        'RAYMAN_NETWORK_RESUME_MAX_ATTEMPTS' = '3'
        'RAYMAN_NETWORK_RESUME_CODEX_PROBE_URL' = 'https://api.openai.com/'
        'RAYMAN_NETWORK_RESUME_COPILOT_PROBE_URL' = 'https://api.github.com/'
        'RAYMAN_NETWORK_RESUME_DESKTOP_SOURCE_ENABLED' = '1'
        'RAYMAN_CODEX_DESKTOP_WATCH_AUTO_START_ENABLED' = '1'
        'RAYMAN_CODEX_DESKTOP_BOOTSTRAP_POLL_MS' = '5000'
        'RAYMAN_CODEX_DESKTOP_ACTIVE_SESSION_SECONDS' = '120'
        'RAYMAN_CODEX_DESKTOP_SESSION_LOOKBACK_HOURS' = '12'
        'RAYMAN_CODEX_DESKTOP_STOP_GRACE_SECONDS' = '900'
        'RAYMAN_CODEX_WORKSPACE_BINDING_AUTO_APPLY_ENABLED' = '1'
        'RAYMAN_CODEX_WORKSPACE_BINDING_AUTO_APPLY_RETRY_SECONDS' = '300'
        'RAYMAN_VSCODE_BOOTSTRAP_PROFILE' = 'conservative'
        'RAYMAN_SCM_CHANGE_SOFT_LIMIT' = '100'
        'RAYMAN_ALLOW_TRACKED_RAYMAN_ASSETS' = '0'
        'RAYMAN_CODEX_LOGIN_FOREGROUND_ONLY' = '1'
        'RAYMAN_CODEX_LOGIN_ALLOW_HIDDEN' = '0'
        'RAYMAN_CODEX_LOGIN_SMOKE_COOLDOWN_MINUTES' = '30'
        'RAYMAN_CODEX_LOGIN_SMOKE_MAX_ATTEMPTS' = '1'
    }
...
"@
    ...
    [void](Invoke-SetupManagedFileWrite `
            -Path $EnvFilePath `
            -Content $newContent `
            -Label 'workspace-env-defaults' `
            -SuccessMessage ("✅ 已补齐 .rayman.env.ps1 默认依赖策略（新增 {0} 项）" -f $missingBlocks.Count) `
            -FallbackMessage ("⚠️ .rayman.env.ps1 被当前会话占用，已使用定长原地写入回退（新增 {0} 项默认依赖策略）。" -f $missingBlocks.Count) `
            -FailurePath '.rayman.env.ps1' `
            -Required)
}
...
function Invoke-EnsureRequirementsLayout {
    ...
    $timeoutSeconds = Get-EnvIntCompat -Name 'RAYMAN_REQUIREMENTS_TIMEOUT_SECONDS' -Default 60 -Min 10 -Max 600
    $tempBase = Join-Path ([System.IO.Path]::GetTempPath()) ('rayman_requirements_' + [Guid]::NewGuid().ToString('N'))
    ...
    try {
        ...
        $proc = $null
        $processParams = @{
            FilePath = [string]$bashCmd.Source
            ArgumentList = @($ensureScriptRel)
        ...
        if (-not $completed) {
            ...
            try {
                $childPids = @(
                    Get-CimInstance Win32_Process -Filter ("ParentProcessId = {0}" -f [int]$proc.Id) -ErrorAction SilentlyContinue |
                    Select-Object -ExpandProperty ProcessId
                )
            } catch {}
            foreach ($childPid in @($childPids | Sort-Object -Descending -Unique)) {
                try { Stop-Process -Id ([int]$childPid) -Force -ErrorAction SilentlyContinue } catch {}
            }
            try { Stop-Process -Id ([int]$proc.Id) -Force -ErrorAction SilentlyContinue } catch {}
            Write-Host ("⚠️ [req] ensure_requirements.sh 超时（>{0}s），已跳过本次补齐。" -f $timeoutSeconds) -ForegroundColor Yellow
            return
        }
...
function Update-RaymanScmIgnoreRules {
    ...
    try {
        ...
        $writeResult = Invoke-SetupManagedFileWrite -Path $targetPath `
            -Content $newContent `
            -Label ("scm-ignore:{0}" -f $mode) `
            -SuccessMessage ("✅ [SCM] 已注入忽略规则（workspace={0}, mode={1}）: {2}" -f $workspaceKind, $mode, $targetPath) `
            -FallbackMessage ("⚠️ [SCM] {0} 被当前会话占用，已使用定长原地写入回退（workspace={1}, mode={2}）。" -f $targetPath, $workspaceKind, $mode) `
...
function Assert-SetupScmIgnoreResult {
    param([object]$Result)
    ...
    if ([string]$Result.reason -ne 'write_failed') {
        return
    }

    $targetPath = ''
    if ($Result.PSObject.Properties['target_path'] -and -not [string]::IsNullOrWhiteSpace([string]$Result.target_path)) {
        $targetPath = [string]$Result.target_path
    } else {
        $targetPath = '[SCM ignore target]'
    }

    $detail = ''
    if ($Result.PSObject.Properties['error_message'] -and -not [string]::IsNullOrWhiteSpace([string]$Result.error_message)) {
        $detail = [string]$Result.error_message
    } else {
        $detail = 'write_failed'
    }

    throw ("managed-write-failed: [SCM] {0} 写入失败（reason=write_failed）：{1}" -f $targetPath, $detail)
}
...
if ([string]::IsNullOrWhiteSpace([string]`$env:RAYMAN_INTERACTION_MODE)) {
    `$env:RAYMAN_INTERACTION_MODE = 'detailed'
}

if ([string]::IsNullOrWhiteSpace([string]`$env:RAYMAN_POST_COMMAND_HYGIENE_ENABLED)) {
...
if ([string]::IsNullOrWhiteSpace([string]`$env:RAYMAN_ALERT_SOUND_ENABLED)) {
    `$env:RAYMAN_ALERT_SOUND_ENABLED = '1'
}

if ([string]::IsNullOrWhiteSpace([string]`$env:RAYMAN_ALERT_SOUND_DONE_ENABLED)) {
    `$env:RAYMAN_ALERT_SOUND_DONE_ENABLED = '1'
}

if ([string]::IsNullOrWhiteSpace([string]`$env:RAYMAN_ALERT_SOUND_PATH)) {
    `$env:RAYMAN_ALERT_SOUND_PATH = (Join-Path ([Environment]::GetFolderPath('UserProfile')) '.rayman\codex\notify.wav')
}
...
if ([string]::IsNullOrWhiteSpace([string]`$env:RAYMAN_AUTO_START_ATTENTION_WATCH_ENABLED)) {
    `$env:RAYMAN_AUTO_START_ATTENTION_WATCH_ENABLED = '1'
}

if ([string]::IsNullOrWhiteSpace([string]`$env:RAYMAN_ALERT_WATCH_ON_PROMPT_ENABLED)) {
    `$env:RAYMAN_ALERT_WATCH_ON_PROMPT_ENABLED = '1'
}

if ([string]::IsNullOrWhiteSpace([string]`$env:RAYMAN_ALERT_WATCH_VSCODE_WINDOWS_ENABLED)) {
    `$env:RAYMAN_ALERT_WATCH_VSCODE_WINDOWS_ENABLED = '1'
}
...
if ([string]::IsNullOrWhiteSpace([string]`$env:RAYMAN_NETWORK_RESUME_WATCH_ENABLED)) {
    `$env:RAYMAN_NETWORK_RESUME_WATCH_ENABLED = '1'
}

if ([string]::IsNullOrWhiteSpace([string]`$env:RAYMAN_NETWORK_RESUME_THRESHOLD_SECONDS)) {
    `$env:RAYMAN_NETWORK_RESUME_THRESHOLD_SECONDS = '1800'
}

if ([string]::IsNullOrWhiteSpace([string]`$env:RAYMAN_NETWORK_RESUME_THROTTLE_WAIT_SECONDS)) {
    `$env:RAYMAN_NETWORK_RESUME_THROTTLE_WAIT_SECONDS = '300'
}

if ([string]::IsNullOrWhiteSpace([string]`$env:RAYMAN_NETWORK_RESUME_POLL_MS)) {
    `$env:RAYMAN_NETWORK_RESUME_POLL_MS = '5000'
}

if ([string]::IsNullOrWhiteSpace([string]`$env:RAYMAN_NETWORK_RESUME_PROBE_TIMEOUT_MS)) {
    `$env:RAYMAN_NETWORK_RESUME_PROBE_TIMEOUT_MS = '5000'
}

if ([string]::IsNullOrWhiteSpace([string]`$env:RAYMAN_NETWORK_RESUME_RETRY_SECONDS)) {
    `$env:RAYMAN_NETWORK_RESUME_RETRY_SECONDS = '300'
}

if ([string]::IsNullOrWhiteSpace([string]`$env:RAYMAN_NETWORK_RESUME_MAX_ATTEMPTS)) {
    `$env:RAYMAN_NETWORK_RESUME_MAX_ATTEMPTS = '3'
}

if ([string]::IsNullOrWhiteSpace([string]`$env:RAYMAN_NETWORK_RESUME_CODEX_PROBE_URL)) {
    `$env:RAYMAN_NETWORK_RESUME_CODEX_PROBE_URL = 'https://api.openai.com/'
}

if ([string]::IsNullOrWhiteSpace([string]`$env:RAYMAN_NETWORK_RESUME_COPILOT_PROBE_URL)) {
    `$env:RAYMAN_NETWORK_RESUME_COPILOT_PROBE_URL = 'https://api.github.com/'
}

if ([string]::IsNullOrWhiteSpace([string]`$env:RAYMAN_NETWORK_RESUME_DESKTOP_SOURCE_ENABLED)) {
    `$env:RAYMAN_NETWORK_RESUME_DESKTOP_SOURCE_ENABLED = '1'
}

if ([string]::IsNullOrWhiteSpace([string]`$env:RAYMAN_CODEX_DESKTOP_WATCH_AUTO_START_ENABLED)) {
    `$env:RAYMAN_CODEX_DESKTOP_WATCH_AUTO_START_ENABLED = '1'
}

if ([string]::IsNullOrWhiteSpace([string]`$env:RAYMAN_CODEX_DESKTOP_BOOTSTRAP_POLL_MS)) {
    `$env:RAYMAN_CODEX_DESKTOP_BOOTSTRAP_POLL_MS = '5000'
}

if ([string]::IsNullOrWhiteSpace([string]`$env:RAYMAN_CODEX_DESKTOP_ACTIVE_SESSION_SECONDS)) {
    `$env:RAYMAN_CODEX_DESKTOP_ACTIVE_SESSION_SECONDS = '120'
}

if ([string]::IsNullOrWhiteSpace([string]`$env:RAYMAN_CODEX_DESKTOP_SESSION_LOOKBACK_HOURS)) {
    `$env:RAYMAN_CODEX_DESKTOP_SESSION_LOOKBACK_HOURS = '12'
}

if ([string]::IsNullOrWhiteSpace([string]`$env:RAYMAN_CODEX_DESKTOP_STOP_GRACE_SECONDS)) {
    `$env:RAYMAN_CODEX_DESKTOP_STOP_GRACE_SECONDS = '900'
}

if ([string]::IsNullOrWhiteSpace([string]`$env:RAYMAN_CODEX_WORKSPACE_BINDING_AUTO_APPLY_ENABLED)) {
    `$env:RAYMAN_CODEX_WORKSPACE_BINDING_AUTO_APPLY_ENABLED = '1'
}

if ([string]::IsNullOrWhiteSpace([string]`$env:RAYMAN_CODEX_WORKSPACE_BINDING_AUTO_APPLY_RETRY_SECONDS)) {
    `$env:RAYMAN_CODEX_WORKSPACE_BINDING_AUTO_APPLY_RETRY_SECONDS = '300'
}

if ([string]::IsNullOrWhiteSpace([string]`$env:RAYMAN_WORKER_AUTO_SYNC_ENABLED)) {
    `$env:RAYMAN_WORKER_AUTO_SYNC_ENABLED = '0'
}

if ([string]::IsNullOrWhiteSpace([string]`$env:RAYMAN_WORKER_AUTO_SYNC_POLL_MS)) {
    `$env:RAYMAN_WORKER_AUTO_SYNC_POLL_MS = '2000'
}

if ([string]::IsNullOrWhiteSpace([string]`$env:RAYMAN_WORKER_AUTO_SYNC_DEBOUNCE_SECONDS)) {
    `$env:RAYMAN_WORKER_AUTO_SYNC_DEBOUNCE_SECONDS = '5'
}

if ([string]::IsNullOrWhiteSpace([string]`$env:RAYMAN_WORKER_AUTO_SYNC_RETRY_SECONDS)) {
    `$env:RAYMAN_WORKER_AUTO_SYNC_RETRY_SECONDS = '30'
}
...
if ([string]::IsNullOrWhiteSpace([string]`$env:RAYMAN_CODEX_LOGIN_FOREGROUND_ONLY)) {
    `$env:RAYMAN_CODEX_LOGIN_FOREGROUND_ONLY = '1'
}
if ([string]::IsNullOrWhiteSpace([string]`$env:RAYMAN_CODEX_LOGIN_ALLOW_HIDDEN)) {
    `$env:RAYMAN_CODEX_LOGIN_ALLOW_HIDDEN = '0'
}
if ([string]::IsNullOrWhiteSpace([string]`$env:RAYMAN_CODEX_LOGIN_SMOKE_COOLDOWN_MINUTES)) {
    `$env:RAYMAN_CODEX_LOGIN_SMOKE_COOLDOWN_MINUTES = '30'
}
if ([string]::IsNullOrWhiteSpace([string]`$env:RAYMAN_CODEX_LOGIN_SMOKE_MAX_ATTEMPTS)) {
    `$env:RAYMAN_CODEX_LOGIN_SMOKE_MAX_ATTEMPTS = '1'
}
"@
    [void](Invoke-SetupManagedFileWrite `
            -Path $workspaceEnvFile `
            -Content $workspaceEnvContent `
            -Label 'workspace-env-bootstrap' `
            -SuccessMessage '✅ 已生成 .rayman.env.ps1（工作区持久配置）' `
            -FallbackMessage '⚠️ .rayman.env.ps1 被当前会话占用，已使用定长原地写入回退。' `
            -FailurePath '.rayman.env.ps1' `
            -Required)
}
...
if ($runtimeCleanupImported -and (Get-Command Invoke-RaymanRuntimeCleanup -ErrorAction SilentlyContinue)) {
    try {
        ...
        if ([int]$runtimeCleanupReport.removed_count -gt 0 -or [int]$runtimeCleanupReport.preserved_count -gt 0) {
            Write-Host ("🧹 [setup] runtime 收敛完成：removed={0}, preserved={1}" -f [int]$runtimeCleanupReport.removed_count, [int]$runtimeCleanupReport.preserved_count) -ForegroundColor DarkCyan
        }
    } catch {
        Write-Host ("⚠️ [setup] runtime 收敛失败：{0}" -f $_.Exception.Message) -ForegroundColor Yellow
    }
}
if ($legacyCleanupImported -and (Get-Command Invoke-RaymanLegacyWorkspaceCleanup -ErrorAction SilentlyContinue)) {
    try {
        $legacyCleanupReport = Invoke-RaymanLegacyWorkspaceCleanup -WorkspaceRoot $WorkspaceRoot
        if ($null -ne $legacyCleanupReport -and [int]$legacyCleanupReport.removed_count -gt 0) {
            Write-Host ("🧹 [setup] 已清理 legacy Rayman 残留：removed={0}" -f [int]$legacyCleanupReport.removed_count) -ForegroundColor DarkCyan
        }
        if ($null -ne $legacyCleanupReport -and [int]$legacyCleanupReport.failed_count -gt 0) {
            Write-Host ("⚠️ [setup] legacy Rayman 残留清理不完整：failed={0}" -f [int]$legacyCleanupReport.failed_count) -ForegroundColor Yellow
            if (-not [string]::IsNullOrWhiteSpace([string]$legacyCleanupReport.warning)) {
                Write-Host ("   {0}" -f [string]$legacyCleanupReport.warning) -ForegroundColor DarkYellow
            }
        }
    } catch {
        Write-Host ("⚠️ [setup] legacy Rayman 残留清理失败：{0}" -f $_.Exception.Message) -ForegroundColor Yellow
    }
}
$script:ScmTrackedNoiseAnalysis = $null
$skipScmChecks = Get-EnvBoolCompat -Name 'RAYMAN_SETUP_SKIP_SCM_CHECKS' -Default $false
...
if ($raymanCommonImported) {
    ...
    if ($skipScmChecks) {
        Write-Host "⏭️ [SCM] 已按环境变量跳过 tracked-noise 预检（RAYMAN_SETUP_SKIP_SCM_CHECKS=1）" -ForegroundColor DarkCyan
    } else {
...
if ($hasCurrentSolutionName -and ($currentSolutionName -eq $targetSolutionName)) {
    ...
    [void](Invoke-SetupManagedFileWrite `
            -Path $solutionNameFile `
            -Content $solutionName `
            -Label 'solution-name' `
            -SuccessMessage '✅ 已生成 .SolutionName' `
            -FallbackMessage '⚠️ .SolutionName 被当前会话占用，已使用定长原地写入回退。' `
            -FailurePath '.SolutionName' `
            -Required)
}
...
请同时读取当前工作区交互模式：优先看 `.Rayman/CONTEXT.md` 的 `Collaboration Preference / 交互偏好`，必要时回读 `.rayman.env.ps1` 中的 `RAYMAN_INTERACTION_MODE=detailed|general|simple`。
...
`full-auto` 只代表代码改动已审批，不代表可以替用户决定需求或在多方案里自行拍板。
...
当用户输入 "保存状态"、"保存状态 <名字>"、"暂停" 或 "暂停 <名字>" 时，请运行 `rayman.ps1 state-save`；优先把名字传给 `-Name`，未提供名字时脚本会自动生成 ``unnamed-<timestamp>``。
...
当用户输入 "一键部署" 时，请运行 `rayman.ps1 deploy`。如果用户指定了项目名（如 "一键部署 WebApp"），请将项目名作为 `-ProjectName` 参数传递给脚本。

【重要】当你在执行任务过程中，遇到需要用户手动确认、输入信息或进行人机交互时，请务必先运行 `.Rayman/scripts/utils/request_attention.ps1` 脚本，通过统一提醒链（声音/提示）提醒用户。你可以通过 `-Message` 参数传递具体的提示内容，例如：`.Rayman/scripts/utils/request_attention.ps1 -Message "需要您确认部署配置"`。
...
[void](Invoke-SetupManagedFileWrite `
        ...
        -SuccessMessage '✅ 已生成 .github/copilot-instructions.md' `
        -FallbackMessage '⚠️ .github/copilot-instructions.md 被当前会话占用，已使用定长原地写入回退。' `
        -FailurePath '.github/copilot-instructions.md' `
        -Required)
...
[void](Invoke-SetupManagedFileWrite `
        ...
        -Content $instructionsContent `
        -Label 'cursor-rules' `
        -SuccessMessage '✅ 已生成 .cursorrules (Codex/Cursor 支持)' `
        -FallbackMessage '⚠️ .cursorrules (Codex/Cursor 支持) 被当前会话占用，已使用定长原地写入回退。' `
        -FailurePath '.cursorrules' `
        -Required)
...
[void](Invoke-SetupManagedFileWrite `
        ...
        -Content $instructionsContent `
        -Label 'cline-rules' `
        -SuccessMessage '✅ 已生成 .clinerules (Cline/Roo Code 支持)' `
        -FallbackMessage '⚠️ .clinerules (Cline/Roo Code 支持) 被当前会话占用，已使用定长原地写入回退。' `
        -FailurePath '.clinerules' `
        -Required)
...
if (Test-Path -LiteralPath $ensureAgentAssetsScript -PathType Leaf) {
    try {
        $previousAgentAssetsForceUpdate = [Environment]::GetEnvironmentVariable('RAYMAN_AGENT_ASSETS_FORCE_UPDATE')
        try {
            if ((Get-RaymanWorkspaceKind -WorkspaceRoot $WorkspaceRoot) -eq 'external') {
                [Environment]::SetEnvironmentVariable('RAYMAN_AGENT_ASSETS_FORCE_UPDATE', '1')
            }
            & $ensureAgentAssetsScript -WorkspaceRoot $WorkspaceRoot | Out-Host
        } finally {
            [Environment]::SetEnvironmentVariable('RAYMAN_AGENT_ASSETS_FORCE_UPDATE', $previousAgentAssetsForceUpdate)
        }
    } catch {
        throw ("agent-assets-failed: {0}" -f $_.Exception.Message)
    }
} else {
    Write-Host ("⚠️ [agent-assets] 缺少脚本：{0}" -f $ensureAgentAssetsScript) -ForegroundColor Yellow
}

$syncDistScript = Join-Path $WorkspaceRoot ".Rayman\scripts\release\sync_dist_from_src.ps1"
if (Test-Path -LiteralPath $syncDistScript -PathType Leaf) {
    try {
        & $syncDistScript -WorkspaceRoot $WorkspaceRoot | Out-Host
    } catch {
        throw ("dist-sync-failed: {0}" -f $_.Exception.Message)
    }
}
...
$raymanTaskDefinitions = @(
    ...
    [ordered]@{
        ...
        Detail = "手动拉起 watcher、exitwatch、worker auto-sync 与后台服务。"
        Command = "powershell"
        Linux = @{ command = "pwsh" }
    ...
    [ordered]@{
        ...
        Detail = "停止 Rayman watcher、worker auto-sync、auto-save 与 MCP 后台服务。"
        Command = "powershell"
        Linux = @{ command = "pwsh" }
...
if (Test-Path -LiteralPath $tasksFile -PathType Leaf) {
    ...
    if ([bool]$tasksDoc.ParseFailed) {
        ...
        [void](Invoke-SetupManagedFileWrite `
                -Path $tasksFile `
                -Content ($existingTasksJson | ConvertTo-Json -Depth 10) `
                -Label 'vscode-tasks-merge' `
                -SuccessMessage '✅ 已更新 .vscode/tasks.json (合并了现有任务)' `
                -FallbackMessage '⚠️ .vscode/tasks.json 被当前会话占用，已使用定长原地写入回退。' `
                -FailurePath '.vscode/tasks.json' `
                -Required)
    }
} else {
        $newTasksObj = @{
    ...
    [void](Invoke-SetupManagedFileWrite `
            -Path $tasksFile `
            -Content ($newTasksObj | ConvertTo-Json -Depth 10) `
            -Label 'vscode-tasks-create' `
            -SuccessMessage '✅ 已生成 .vscode/tasks.json' `
            -FallbackMessage '⚠️ .vscode/tasks.json 被当前会话占用，已使用定长原地写入回退。' `
            -FailurePath '.vscode/tasks.json' `
            -Required)
}
...
if (Test-Path -LiteralPath $launchFile -PathType Leaf) {
    ...
    if ([bool]$launchDoc.ParseFailed) {
        ...
        [void](Invoke-SetupManagedFileWrite `
                -Path $launchFile `
                -Content ($launchObj | ConvertTo-Json -Depth 12) `
                -Label 'vscode-launch-merge' `
                -SuccessMessage '✅ 已更新 .vscode/launch.json (合并了现有调试配置)' `
                -FallbackMessage '⚠️ .vscode/launch.json 被当前会话占用，已使用定长原地写入回退。' `
                -FailurePath '.vscode/launch.json' `
                -Required)
    }
} else {
    $newLaunchContent = @{
    ...
    [void](Invoke-SetupManagedFileWrite `
            -Path $launchFile `
            -Content $newLaunchContent `
            -Label 'vscode-launch-create' `
            -SuccessMessage '✅ 已生成 .vscode/launch.json' `
            -FallbackMessage '⚠️ .vscode/launch.json 被当前会话占用，已使用定长原地写入回退。' `
            -FailurePath '.vscode/launch.json' `
            -Required)
}
...
"@
    ...
    [void](Invoke-SetupManagedFileWrite `
            -Path $settingsPath `
            -Content ($settingsObj | ConvertTo-Json -Depth 5 -Compress:$false) `
            -Label 'vscode-settings' `
            -SuccessMessage '✅ 已配置 .vscode/settings.json 深度性能优化及 Codex 宿主运行环境' `
            -FallbackMessage '⚠️ .vscode/settings.json 被当前会话占用，已使用定长原地写入回退。' `
            -FailurePath '.vscode/settings.json' `
            -Required)
} catch {
    if ($_.Exception.Message -like 'managed-write-failed:*') {
        throw
    }
...
Write-Host ""
if ($skipScmChecks) {
    Write-Host "⏭️ [SCM] 已按环境变量跳过拷贝后噪声忽略与变更统计（RAYMAN_SETUP_SKIP_SCM_CHECKS=1）" -ForegroundColor DarkCyan
} else {
    ...
    Assert-SetupScmIgnoreResult -Result $scmIgnoreResult
