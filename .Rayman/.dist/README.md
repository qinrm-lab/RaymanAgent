...
Rayman 现在支持 `开发机 -> 工作机` 的局域网 worker 链路。v1 假设环境是**受控独立局域网**，因此默认允许 discovery 和控制链路在不配置 worker 口令时直接工作；如果后续进入普通办公网、跨网段或不可信网络，必须显式配置 auth / pairing。
...
1. 建议在开发机 `.rayman.env.ps1` 固定 `$env:RAYMAN_WORKER_SYNC_MODE = 'staged'`；只有在受控 LAN 之外或需要额外保护时，才额外配置与工作机一致的 `$env:RAYMAN_WORKER_AUTH_TOKEN`。
...
- attached：工作机直接使用自己已有的 repo/worktree
  - `rayman.ps1 worker sync --mode attached`
...
1. 先执行 `rayman.ps1 worker status`，确认 `debugger_ready=true`，失败时查看 `debugger_error`
...
- `[ pwsh-only ]` `rayman.ps1 memory-search`：Query Agent Memory hints and recent summaries.
#### Diagnostics
